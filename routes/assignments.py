...
def transfer_device():
    ...
    try:
        device_barcode = request.form.get('device_barcode')
        from_nurse_barcode = request.form.get('from_nurse_barcode')
        to_nurse_barcode = request.form.get('to_nurse_barcode')
        
        if not all([device_barcode, from_nurse_barcode, to_nurse_barcode]):
            return jsonify({'error': 'All barcodes are required'}), 400
        
        db_service = DBService()
        
        # Get device and nurses
        device = db_service.get_device_by_barcode(device_barcode)
        from_nurse = db_service.get_nurse_by_barcode(from_nurse_barcode)
        to_nurse = db_service.get_nurse_by_barcode(to_nurse_barcode)
        
        if not all([device, from_nurse, to_nurse]):
        ...
        if not current_assignment or current_assignment.nurse_id != from_nurse.id:
        ...
        current_assignment.transferred_to_nurse_id = to_nurse.id
        ...
        # Update device
        device.assigned_to = to_nurse.id
        ...
        return jsonify({
            'success': True,
            'message': f'Device transferred from {from_nurse.name} to {to_nurse.name}',
